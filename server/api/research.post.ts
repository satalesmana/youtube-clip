 ...
 * Body (all optional):
 * ```json
 * { "max_trends": 10, "language": "id", "subreddits": "worldnews,indonesia", "rss_feeds": "..." }
 * ```
 *
 * Collects viral signals from news RSS, Reddit, Google Trends and X, ranks
 * them with the LLM, and matches YouTube videos for each topic.
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const result = await researchController.research();
